...
def InitFn():
    ...
    if os.path.exists(TARGET):
        ...
        return
    print "Creating DB ..."
    headers = requests.get('https://docs.microsoft.com/en-us/windows/win32/api/toc.json').json()
    for hdr in headers['items'][0]['children'][1]['children']:
        title = hdr['toc_title'].decode()
        print title
        data = requests.get('https://docs.microsoft.com/en-us/windows/win32/api/%s/toc.json' % hdr['href'].decode()).json()
